export const NavigationTabs = {
	Home: { name: 'homeTab', title: 'Home' },
	Plants: { name: 'plantsTab', title: 'My Plants' },
	Profile: { name: 'profileTab', title: 'Profile' },
	Search: { name: 'searchTab', title: 'Search' },
};

export const NavigationScreens = {
	SignIn: { name: 'signin', title: 'Sign In' },
	SignUp: { name: 'signup', title: 'Sign Up' },
	ResetPassword: { name: 'resetpassword', title: 'Forgot Password?' },
	Home: { name: 'home', title: 'Home' },
	Plants: { name: 'plants', title: 'My Plants' },
	AddPlant: { name: 'addplant', title: 'Add Plant' },
	Households: { name: 'households', title: 'My Households' },
	AddHousehold: { name: 'addhousehold', title: 'Add Household' },
	Search: { name: 'search', title: 'Discover a new Plant' },
	EditPlant: { name: 'editplant', title: 'Edit Plant' },
	Profile: { name: 'profile', title: 'My Profile' },
	Info: { name: 'Information', title: 'More Information' },
	Invite: { name: 'invite', title: 'Invite User' }
};
